...
export default function AddUserDialog({
  ...
  console.log(session?.accessType)

  const resetMutation = useMutation({
    mutationFn: async (values: { email: string }) => {
      const response = await base.post(`/trial/shop/invite-user/`, {
        email: email, accessType: session?.accessType
      });
      return response;
    },
      ...
      <DialogTitle sx={{ padding: "0px", paddingBottom: "24px" }}>
        <div className=" flex flex-col">
          <div className="font-semibold">
            Add <span className="font-normal italic">User</span>
          </div>
      ...
      <DialogContent sx={{ padding: "0px", paddingTop: "8px" }}>
        <div className="flex flex-col gap-y-5 justify-between items-center">
          <TextField
            ...
            label="Email address of staff user"
            autoComplete="email"
            type="email"
            variant="outlined"
            value={email}
          ...
          <div className="text-xs font-normal">
            The added staff member will be provided with the same access as
            yourself, including the QR code scanner and ability to redeem your
            customers drinks. <br /><br />
